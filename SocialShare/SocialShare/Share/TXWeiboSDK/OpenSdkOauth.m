...
- (void) doWebViewAuthorize:(UIWebView *)webView {
	...
	TXAuthorizeWebView *txAuthorizeWebView = [[TXAuthorizeWebView alloc] init];
    [txAuthorizeWebView setDelegate:self];
    [txAuthorizeWebView loadRequestWithURL:[NSURL URLWithString:loadingURL]];
    [txAuthorizeWebView show:YES];
    [txAuthorizeWebView release];
}

- (void) doWebViewAuthorizeWithDelegate:(id)delegateObj{
    delegate=delegateObj;
    [self doWebViewAuthorize:nil];
}
...
- (void) oauthDidSuccess:(NSString *)accessToken accessSecret:(NSString *)accessSecret openid:(NSString *)openid openkey:(NSString *)openkey expireIn:(NSString *)expireIn {
    ...
    //[((TXSocialShare *)delegate) userInfo:((TXSocialShare *)delegate).selfDelegate];
...
- (void) oauthDidFail:(uint16_t)oauthType success:(BOOL)success netNotWork:(BOOL)netNotWork {
    ...
    if (oauthType == InAuth1 && success ) {
        ...
        NSLog(@"oauth type is %hu", oauthType);
    }
}
...
-(BOOL)isLoggedIn{
    ...
    NSLog(@"%@",self.accessSecret);
    NSLog(@"%@",self.openid);
    NSLog(@"%@",self.openkey);
    NSLog(@"%@",self.expireIn);
    if (self.accessToken!=nil&&self.openid!=nil&&self.openkey!=nil) {
        result=YES;
    }
    return result;
}
...
-(void)logOut{
    ...
    self.openkey = nil;
    self.expireIn = nil;
    NSLog(@"cancel to authorize");
    [OpenSdkBase showMessageBox:@"取消授权成功！"];
